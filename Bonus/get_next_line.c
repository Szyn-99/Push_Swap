...
/*   Updated: 2025/12/19 23:33:04 by aymel-ha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*process_reading_helper(char *storage, char *temp)
{
	...
	if (!storage && !temp)
		...
		return (gnl_strdup(temp));
	else if (!temp)
	{
		concatenated_results = gnl_strdup(storage);
		free(storage);
		return (concatenated_results);
	}
	else
	{
		concatenated_results = gnl_strjoin(storage, temp);
		return (concatenated_results);
	}
	return (NULL);
}
...
char	*process_storage_resize(char *storage)
{
	...
	if (storage[i] == '\n')
		i++;
	resized_storage = malloc(gnl_strlen(storage) - i + 1);
	while (storage[i])
	{
